...
本指南适用于所有针对 `thinkgos/home-via-nix` 的 AI辅助贡献，其中介绍了该项目的架构、命令及开发工作流。
...
### Ansible 系统包管理
- 前置条件: `home-manager` 已经切换到用户配置, 它包含了 `ansible` 命令.
...
### 配置结构
- `flake.nix`：入口点，定义三个配置文件：
  - `thinkgo-fedora43`：thinkgo 用户在 Fedora 43 上
  - `thinkgo-ubuntu25_10`：thinkgo 用户在 Ubuntu 25.10 上
  - `journey-ubuntu25_10`：journey 用户在 Ubuntu 25.10 上
...
- `roles/`：用于外部工具安装的 Ansible roles。
  - `system/`：系统包, 包含发行版特定变量。
  - `github/`：GitHub 应用安装，支持版本跟踪和包/二进制策略。
- `site.yml`：Ansible playbook入口。

### `home-manager`的 Flake 结构
...
- `roles/system`：系统包, 包含发行版特定变量, 其中vars文件, `main.yml`为通用包, `deb.yml`和`rpm.yml`为发行版特定包。
- `roles/github`： GitHub 应用安装角色, 管理 GitHub 对应版本应用的安装。
...
### 添加新 GitHub 应用时
1. 在 `roles/github/vars/main.yml` 中添加条目，包含必需字段（name、repo、version、tag_v）。
